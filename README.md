...
> **3. 建议开启 Antigravity 的自动同意（Auto-approve）设置**
>
> Antigravity 内置了安全机制，部分操作（如执行终端命令、修改文件等）需要用户手动点击确认。如果你希望飞书消息能全自动触发 Agent 处理任务，建议在 Antigravity 设置中将相关确认项调整为自动同意，以最大化自动化体验。但请注意，目前仍有少数操作无法跳过人工确认（这是 Antigravity 的安全底线），遇到这类情况时需要手动介入。另外，文件变更后弹出的「Accept all」提示不影响任务执行，可以忽略。
